                ...
                match source.next().await {
                    Some(Ok(event)) => {
                        ...
                        if let Ok(value) = serde_json::from_str::<Value>(data) {
                            if let Some(error) = value.get("error") {
                                let message = error
                                    ...
                                    .unwrap_or("deepseek stream error");
                                return Some((
                        ...
                        match parsed {
                            ...
                            Err(err) => {
                                return Some((
                                    Err(AppError::Decode(format!("bad deepseek event: {err}"))),
                                    (source, parser, pending, true, saw_any_event),
                                ));
                            }
                        }
                    ...
                    None => {
                        if !saw_any_event {
                            return Some((
                                Err(AppError::Stream(
                                    "deepseek SSE closed before any event; \
            ...
            if let Some(state) = self.tool_states.remove(&key) {
                if let Some(index) = state.part_index {
                    out.push(StreamEvent::PartMeta {
                        index,
                        meta: json!({ "provider": "deepseek", "id": state.id, "name": state.name }),
                    });
                    out.push(StreamEvent::PartStop { index });
                }
            }
