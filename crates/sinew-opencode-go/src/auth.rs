...
pub struct OpenCodeGoAuthStatus {
    pub connected: bool,
    pub key_preview: Option<String>,
    pub last_validated_ms: Option<i64>,
}

impl OpenCodeGoAuthStatus {
...
impl Credential {
    ...
    pub fn from_sinew_auth_file(path: &Path) -> Result<Option<Self>> {
        ...
        if api_key.is_empty() {
            return Err(AppError::Auth("OpenCode Go auth is missing API key".into()));
        }
...
pub fn save_default_api_key(api_key: &str) -> Result<OpenCodeGoAuthStatus> {
...
pub fn touch_default_auth_validation() -> Result<OpenCodeGoAuthStatus> {
    let path = default_auth_path()?;
    let bytes = match std::fs::read(&path) {
        Ok(bytes) => bytes,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return Ok(OpenCodeGoAuthStatus::disconnected())
        }
        Err(err) => return Err(AppError::Auth(format!("unable to read auth file: {err}"))),
    };
...
pub fn load_default_auth_status() -> Result<OpenCodeGoAuthStatus> {
    load_auth_status(&default_auth_path()?)
}

pub fn load_auth_status(path: &Path) -> Result<OpenCodeGoAuthStatus> {
    let bytes = match std::fs::read(path) {
        Ok(bytes) => bytes,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return Ok(OpenCodeGoAuthStatus::disconnected())
...
fn default_auth_path() -> Result<PathBuf> {
    ...
    Ok(dirs.data_local_dir().join("opencode-go-auth.json"))
}

fn status_from_auth(auth: &StoredAuth) -> OpenCodeGoAuthStatus {
    if auth.provider != PROVIDER_ID || auth.auth_mode != "api_key" {
        return OpenCodeGoAuthStatus::disconnected();
    }
