...
use chromiumoxide::cdp::browser_protocol::network::{
    CookieSameSite, DeleteCookiesParams, EventRequestWillBeSent, EventResponseReceived,
    GetCookiesParams, SetCookieParams,
};
...
impl BrowserSession {
    pub async fn launch(headless: bool) -> Result<Self> {
        ...
        // chromiumoxide's BrowserConfig::builder() defaults to headless
        // (it injects `--headless` itself), so passing `headless = false`
        // from the agent wasn't enough — we have to explicitly opt back
        // into a head with `.with_head()`. When the agent really wants
        // headless, we pass `--headless=new` (the new Chrome headless
        // mode that supports more APIs than the legacy one).
        if headless {
    ...
    pub async fn screenshot(&self, full_page: bool) -> Result<Vec<u8>> {
        ...
        // A `full_page` screenshot of a long page can exceed the 8000 px that
        // LLM providers (Anthropic, etc.) accept, which would poison the chat.
        // Clamp the longest side so the image stays usable. Done off the async
        // runtime since the resize is CPU-bound.
        let bytes = tokio::task::spawn_blocking(move || downscale_if_oversized(bytes)).await?;
        Ok(bytes)
    }
    ...
    pub async fn wait_for_with_hint(
        &self,
        selector: &str,
        state: &str,
        timeout_ms: u64,
        ...
        if found {
            ...
            Ok(format!(
                "Timeout waiting for '{selector}' (state={state})\n\n{hint}"
            ))
        }
    }
    ...
    pub async fn hover(&self, selector: &str) -> Result<()> {
        let el = self
            ...
            .await
            .map_err(|_| anyhow::anyhow!("Element not found: {selector}"))?;
        el.hover().await?;
        Ok(())
    }
  ...
  return results;
    ...
    pub async fn pdf(&self) -> Result<Vec<u8>> {
        ...
        let result = self
            .page
            .execute(chromiumoxide::cdp::browser_protocol::page::PrintToPdfParams::default())
            .await?;
    ...
    pub async fn send_keys(&self, key: &str) -> Result<()> {
        ...
        let (key_id, code, text) = match key_lower.as_str() {
            ...
            "ctrl+c" => {
                self.page
                    .evaluate("document.execCommand('copy')")
                    .await
                    .ok();
                return Ok(());
            }
            "ctrl+v" => {
                self.page
                    .evaluate("document.execCommand('paste')")
                    .await
                    .ok();
                return Ok(());
            }
...
fn find_browser_executable() -> Option<std::path::PathBuf> {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            owned_candidates.push(format!(r"{local}\Google\Chrome\Application\chrome.exe"));
            owned_candidates.push(format!(r"{local}\Microsoft\Edge\Application\msedge.exe"));
        }
    }
    ...
    let candidates: Vec<&str> = owned_candidates
        .iter()
        .map(|s| s.as_str())
...
/// Longest side we allow a screenshot to be. Kept under the 8000 px ceiling
/// that LLM providers (Anthropic, etc.) enforce on uploaded images.
const MAX_SCREENSHOT_DIMENSION: u32 = 7800;

/// Downscale a PNG screenshot so neither side exceeds `MAX_SCREENSHOT_DIMENSION`,
/// preserving aspect ratio. Returns the original bytes unchanged if it already
/// fits or if decode/encode fails (fail-open — a screenshot beats no screenshot).
fn downscale_if_oversized(png: Vec<u8>) -> Vec<u8> {
    let Ok(img) = image::load_from_memory(&png) else {
        return png;
    };
    if img.width() <= MAX_SCREENSHOT_DIMENSION && img.height() <= MAX_SCREENSHOT_DIMENSION {
        return png;
    }
