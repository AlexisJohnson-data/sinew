...
impl BrowserTools {
    ...
    async fn run_open(&self, input: Value) -> ToolRunResult {
        ...
        if !sessions.contains_key(&self.workspace_id) {
            match BrowserSession::launch(parsed.headless).await {
                Ok(session) => {
                    sessions.insert(self.workspace_id.clone(), session);
                }
                Err(e) => return ToolRunResult::err(e.to_string(), Vec::new()),
            }
        }
    ...
    async fn run_type(&self, input: Value) -> ToolRunResult {
        ...
        match session
            .type_text(&parsed.selector, &parsed.text, parsed.clear)
            .await
        {
            Ok(()) => ToolRunResult::ok(
    ...
    async fn run_network(&self, input: Value) -> ToolRunResult {
        ...
        match session
            .network(parsed.clear, parsed.filter.as_deref())
            .await
        {
            Ok(entries) => {
    ...
    async fn run_wait(&self, input: Value) -> ToolRunResult {
        ...
        match session
            .wait_for_with_hint(&parsed.selector, &parsed.state, parsed.timeout_ms)
            .await
        {
    ...
    async fn run_scroll(&self, input: Value) -> ToolRunResult {
        ...
        match session
            .scroll(parsed.selector.as_deref(), parsed.x, parsed.y)
            .await
        {
    ...
    async fn run_record_stop(&self, input: Value) -> ToolRunResult {
        ...
        match handle.stop(format).await {
            Ok((bytes, media_type, frame_count, duration_ms)) if frame_count > 0 => {
                ...
                ToolRunResult::ok_with_images(
                    format!(
                        "{frame_count} frames · {duration_ms}ms · {} KB · {media_type}",
                        bytes.len() / 1024
                    ),
                    vec![image],
                    Vec::new(),
            ...
            Ok(_) => {
                ToolRunResult::err("Recording stopped but no frames were captured", Vec::new())
            }
            Err(e) => ToolRunResult::err(e.to_string(), Vec::new()),
        }
    }
    ...
    async fn run_find(&self, input: Value) -> ToolRunResult {
        ...
        match session
            .find_by_text(&parsed.text, parsed.role.as_deref())
            .await
        {
        ...
        match parsed.action.as_str() {
            ...
            "set" => {
                let name = match &parsed.name {
                    Some(n) => n.as_str(),
                    None => return ToolRunResult::err("name required for set", Vec::new()),
                };
                let value = match &parsed.value {
                    Some(v) => v.as_str(),
                    None => return ToolRunResult::err("value required for set", Vec::new()),
                };
                let domain = match &parsed.domain {
                    Some(d) => d.as_str(),
                    None => return ToolRunResult::err("domain required for set", Vec::new()),
                };
                match session
                    .set_cookie(name, value, domain, parsed.path.as_deref())
                    .await
                {
                    Ok(()) => {
                        ToolRunResult::ok(format!("Cookie '{name}' set on {domain}"), Vec::new())
                    }
                    Err(e) => ToolRunResult::err(e.to_string(), Vec::new()),
                }
            }
            "delete" => {
                let name = match &parsed.name {
                    Some(n) => n.as_str(),
                    None => return ToolRunResult::err("name required for delete", Vec::new()),
                };
    ...
    async fn run_iframe(&self, input: Value) -> ToolRunResult {
        ...
        match parsed.action.as_str() {
            "eval" => {
                let js = match &parsed.js {
                    Some(j) => j.as_str(),
                    None => return ToolRunResult::err("js required for eval", Vec::new()),
                };
            ...
            "click" => {
                let sel = match &parsed.selector {
                    Some(s) => s.as_str(),
                    None => return ToolRunResult::err("selector required for click", Vec::new()),
                };
                match session.click_in_iframe(&parsed.iframe_selector, sel).await {
                    Ok(()) => ToolRunResult::ok(
                        format!(
                            "Clicked '{}' inside iframe '{}'",
                            sel, parsed.iframe_selector
                        ),
                        Vec::new(),
                    ),
                    Err(e) => ToolRunResult::err(e.to_string(), Vec::new()),
...
fn save_pdf(workspace_id: &str, bytes: &[u8]) -> Option<String> {
    let dir = std::path::Path::new(workspace_id)
        .join(".sinew")
        .join("screens");
